...
def create_lmdb(partitions):
    caffe_dir = '/home/nitini/caffe-master'
    convert_imageset = caffe_dir + '/build/tools/convert_imageset'
...
def train_caffe_seaNet(seaNet_solver):
    caffe_dir = '/home/nitini/caffe-master'
    train_cmd = caffe_dir + '/build/tools/caffe train'
...
def main():
    seaNet_solver = sys.argv[1]
    k = int(sys.argv[2])
    training_data_folder = sys.argv[3]
    fold_map = {}
    for i in range(k):
        fold_map[i] = []
    ...
    for i in range(k):
        ...
        break
        #train_caffe_seaNet(seaNet_solver)
